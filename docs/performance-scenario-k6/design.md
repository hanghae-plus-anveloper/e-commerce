...
- 동시 최대 2000명의 사용자가 핵심 플로우를 수행하는 동안의 성능 및 안정성을 검증함을 목적으로함.
- 핵심 플로우는 잔액 충전 및 사용자 ID 발급 → 한정 수량에 대한 쿠폰 발급 시도 → 여러 상품을 한번에 구매 순으로 진행함.
- 잔액 충전은 각자 정상적으로 충전이 되며, 쿠폰 발급은 2000개의 요청 중 1000건만 성공하고, 쿠폰 사용을 포함하여 2000건의 주문이 생성되어야 함.
...
- 각 유형별 1000건의 사용자가 잔액 충전을 통해 ID을 발급 받아 3000건의 사용자 테이블이 생성 되는 지 확인
- 모두 쿠폰 발급 시도를 하되, 1000개의 쿠폰만 발급이 되는 지 확인, 유형별 쿠폰에 따라 일부 주문 정보가 달라지는 지 확인
- 주문 생성의 시도는 각 유형별 1000건을 시도하되, 정상적으로 구매를 하는 2000건만 성공
- 사용된 잔액이 예측했던 금액인 지 확인
- 테스트 속도에 따라 각 유형 1000명에 대한 가상 사용자 수 조정
